...
std::unique_ptr<Texture> load_texture(const std::string &file, bool flip) {
  ...
  if (stbi_is_hdr(file.c_str())) {
    ...
    memcpy(data.data(), pixel_data, width * height * 4 * sizeof(float));
    ...
    memcpy(data.data(), pixel_data, width * height * 4 * sizeof(uint8_t));
    texture->data = data;
    stbi_image_free(pixel_data);
  }
  texture->width = width;
...
std::unique_ptr<TextureCube>
load_texture_cube(const std::vector<std::string> files, bool flip) {
  ...
  for (int i = 0; i < 6; i++) {
    if (stbi_is_hdr(files[i].c_str())) {
      float *pixel_data =
          stbi_loadf(files[i].c_str(), &width, &height, &_ignore, 4);
      if (!pixel_data) {
      ...
      uint8_t *pixel_data =
          stbi_load(files[i].c_str(), &width, &height, &_ignore, 4);
      if (!pixel_data) {
        spdlog::error("Failed to load texture: {}", files[i]);
        throw std::runtime_error("Failed to load texture: " + files[i]);
      }
