...
GLTexture::GLTexture(const HDRTexture &texture) {
  set_id(texture.id());
  GLenum target = GL_TEXTURE_2D;
...
GLTextureCube::GLTextureCube(const std::vector<Texture> &textures) {
  GLenum target = GL_TEXTURE_CUBE_MAP;

  glCreateTextures(target, 1, &m_handle);
  glTextureStorage2D(m_handle, 1, GL_RGBA8, textures[0].width,
                     textures[0].height);
  for (int i = 0; i < 6; i++) {
    glTextureSubImage3D(m_handle, 0, 0, 0, i, textures[i].width,
